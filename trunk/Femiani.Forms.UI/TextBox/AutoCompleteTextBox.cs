...
namespace CustomForm
{
	...
	public class AutoCompleteTextBox : System.Windows.Forms.TextBox
	{
        ...
        /// <summary>
        /// Whether the string used to display is also used for matching purpose
        /// </summary>
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            switch (keyData)
            {
                ...
                //case Keys.PageUp:
                //    {
                //        //TODO: Implement page up/down scrolling
                //        //this.Mode = EntryMode.List;
                //    } break;
                default:
                    {
                        val = DefaultCmdKey(ref msg, keyData);
                    } break;
            }
        ...
        protected override void OnTextChanged(EventArgs e)
        {
            ...
            if (!this.popup.Visible) SelectedItem = null;
		...
		public AutoCompleteTextBox()
		{
			...
			this.triggers.Add(new ShortCutTrigger(Keys.Tab, TriggerState.SelectAndConsume));
            ...
            this.triggers.Add(new ShortCutTrigger(Keys.Down, TriggerState.Show));

            this.MatchMode = MatchModes.Contains;
            this.MatchDisplayString = true;
        }
		...
		protected virtual object[] FilterList()
		{
            AutoCompleteEntryCollection newList = new AutoCompleteEntryCollection();
            AutoCompleteEntryCollection list = this.Items;
            foreach (IAutoCompleteEntry entry in list)
            {
                if (this.MatchDisplayString)
                {
                    if ((MatchMode == MatchModes.StartsWith && entry.ToString().ToUpper().StartsWith(this.Text.ToUpper()))
                                ||
                       (MatchMode == MatchModes.Contains && entry.ToString().ToUpper().Contains(this.Text.ToUpper())))
                    {
                        newList.Add(entry);
                        continue;
                    }
                ...
                {
                        if (MatchMode == MatchModes.StartsWith && match.ToUpper().StartsWith(this.Text.ToUpper())
                            ||
                            MatchMode == MatchModes.Contains && match.ToUpper().Contains(this.Text.ToUpper()))
                        {
                            newList.Add(entry);
                            break;
                        }
